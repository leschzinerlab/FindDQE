...
C**************************************************************************
      IMPLICIT NONE
C
      INTEGER*4 I,HDR,IM,IFILE,MODE,NX,NY,NZ,MAPCRS(3)
      ...
      CHARACTER CFORM,CSTIN,CF(IM),CNAME*200
...
C
      IOMODE(IFILE)=0
      IF (CSTIN.EQ.'N') IOMODE(IFILE)=1
      IF (CSTIN.EQ.'n') IOMODE(IFILE)=1
      IF (CSTIN.EQ.'U') IOMODE(IFILE)=1
      IF (CSTIN.EQ.'u') IOMODE(IFILE)=1
...
C
        IF (LABBYT.GT.HDR) THEN
          DO 50 I=1,LABBYT-HDR
            CALL CWRITE(BB,HDR+I,1,1,IFILE)
50        CONTINUE 
        ENDIF
C
        RL(IFILE)=NXYZ(1,IFILE)*4
...
C
      IMPLICIT NONE
C
      INTEGER*4 I,IFILE,IM,IREC,MAPCRS(3)
      PARAMETER (IM=200)
...
C
      IMPLICIT NONE
C
      INTEGER*4 I,IFILE,IM,IREC,I4,I2,MAPCRS(3)
      PARAMETER (IM=200)
      INTEGER*4 NXYZ(3,IM),MO(IM),NLABL(IM)
...
C
      IMPLICIT NONE
C
      INTEGER*4 HDR,IM,I,SLEN,MAPCRS(3)
      PARAMETER (HDR=1024,IM=200)
...
C
      ...
      PARAMETER (HDR=1024)
      INTEGER*4 NXYZ(3),MODE,EXTRA1(7),EXTRA2(53),MAPCRS1(3)
...
C
      ...
      INTEGER*4 ISPG,NSYMBT,EXTRA(25),NLABL,MAPCRS1(3)
